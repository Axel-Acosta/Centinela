...
- Current release-source-check result: `py-dncp-release-source-check` persists official DNCP OCDS release packages and document metadata as source records for entity casework. Current live state is 10 release package records and 1,462 document metadata records across `MENDEZ GONZALEZ FLORIANA *`, `CONSULTORA GUARANI SA INGENIEROS CIVILES`, `PROSALUDFARMA S.A.`, `INDEX S.A.C.I.`, and `QUIMFA S.A.`.
- Current document-content result: `py-dncp-document-content` persists captured official DNCP documents and bounded extraction attempts as source records. Current live state is 8 document-content records: 2 captured official DNCP contract PDFs across `MENDEZ GONZALEZ FLORIANA *` and `CONSULTORA GUARANI SA INGENIEROS CIVILES` that downloaded, hashed, and returned `no_extractable_text`, plus 6 priority-company contract attempts for `PROSALUDFARMA S.A.`, `INDEX S.A.C.I.`, and `QUIMFA S.A.` where DNCP returned `404` and the source-access limitation is preserved.
- Current entity source-pack result: `npm run database:entity-source-pack` and `POST /api/entities/:id/source-packs` now create/reuse analyst cases from entity-linked source records, add source-record evidence links, and write evidence artifacts, source manifests, source bundles, and source-document indexes in one command. Current live state includes case `19` for `MENDEZ GONZALEZ FLORIANA *` with 8 evidence links, case `20` for `CONSULTORA GUARANI SA INGENIEROS CIVILES` with 10 evidence links, and cases `22`, `23`, and `24` for `PROSALUDFARMA S.A.`, `INDEX S.A.C.I.`, and `QUIMFA S.A.` with 12 evidence links each after document-content limitation records were added.
- Current source-pack readiness result: `npm run database:entity-source-pack-readiness -- --limit 15 --source-record-limit 10` ranks the next rollout queue. The latest top-three priority companies are `ready_for_internal_review_with_document_download_limits`; the next 12 ranked entities still need `py-dncp-release-source-check`.
- Next expansion: supplier histories enriched with non-DNCP sources, the next lawful company/officer/ownership-adjacent accountability source, readiness-ranked DNCP release/document source checks for the next high-priority entities, formal DNCP crosswalk refinement, and selective alternate document access/OCR only where case-priority source packs justify it
