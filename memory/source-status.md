...
- Current entity source-pack result: `npm run database:entity-source-pack` and `POST /api/entities/:id/source-packs` now create/reuse analyst cases from entity-linked source records, add source-record evidence links, and write evidence artifacts, source manifests, source bundles, and source-document indexes in one command. Current live state includes case `19` for `MENDEZ GONZALEZ FLORIANA *` with 8 linked source records/evidence links and case `20` for `CONSULTORA GUARANI SA INGENIEROS CIVILES` with 10 linked source records/evidence links; the Mendez rerun skipped all 8 existing evidence links as intended.
- Next expansion: supplier histories enriched with non-DNCP sources, broader high-priority DNCP release/document source checks, selective document-content capture/OCR where lawful and useful, formal DNCP crosswalk refinement, and console support for entity source-pack generation
