...
- Interpretation: the local identity bottleneck is much weaker now, exact matching is no longer the only output path, review state is operational, and source-document evidence has been captured. The main remaining bottleneck is accepted-match governance: Centinela needs a second-review workflow before any `promotable` candidate can enter accepted-match or external-risk tables.
- Next expansion: build the second-review accepted-match workflow for `promotable` candidates, improve the local candidate scoring/evidence model, then add ownership/foreign-company/offshore edges on top of the DNCP plus DNIT company master.
