...
- Current manual-review state: 58 OpenSanctions candidate/diagnostic records are visible in `centinela.entity_enrichment_candidate_review_overview`; 1 is `promotable`, 5 are `monitor`, 4 are `rejected`, and 48 remain `unreviewed`.
- Current active external candidate lead: candidate `59`, `CONSULTORA GUARANI SA INGENIEROS CIVILES` -> `Consultora Guaraní S.A. Ingenieros Civiles`, is marked `promotable`. It has local DNCP/DNIT RUC evidence including `80016063-0`, official IDB row-level evidence, and hosted same-candidate support, but no comparable external RUC identifier.
...
- Second-review implementation status: migration `sql/postgres/015_external_candidate_second_review.sql` and command `npm run database:second-review-external-candidate` exist in the repo, but live VPS application is pending until the local SSH tunnel is reopened.
- Interpretation: the local identity bottleneck is much weaker now, exact matching is no longer the only output path, review state is operational, source-document evidence has been captured, and accepted-match governance now exists in code/schema. The live database still needs migration 015 applied before candidate `59` can be second-reviewed.
- Next expansion: apply migration 015, dry-run and then decide candidate `59`, improve the local candidate scoring/evidence model, then add ownership/foreign-company/offshore edges on top of the DNCP plus DNIT company master.
