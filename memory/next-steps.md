...
20. Move to the next intelligence source:
   ...
   The privacy-safe person-relationship staging lane now exists: apply `sql/postgres/020_abogacia_relationship_staging.sql`, then run `npm run enrichment:abogacia-person-relationship-staging -- --limit 250`. It stages redacted director/shareholder/beneficial-owner leads without storing raw person names, document numbers, addresses, or raw full CSV files.
...
24. Use the current progress estimate as the planning baseline:
   `docs/execution/progress-and-remaining-work.md` estimates the analyst/casework source-pack phase is complete enough for internal use, the presentable local/internal product surface is complete enough for internal MVP use unless a concrete blocker appears, and the first serious Paraguay internal MVP still needs about 3-7 focused implementation runs.
...
29. Treat Abogacia person-level data as review-only:
   directors, shareholders, and beneficial-owner CSVs are publicly discoverable, but they include personal fields. Centinela may use the new staging lane only under the documented minimization boundary: redacted display, one-way name hash, source row hash, no raw person names, no document/address fields, `blocked_personal_data`, and no promotion to person entities without separate governance.
...
31. Next best backend move after the first Abogacia staging pilot:
   live source run `52` staged 90 redacted Abogacia relationship leads and CODEX S.R.L. confirms the dossier/API/network path works. Inspect staged rows for usefulness, then either widen staging beyond the pilot limit or build a promotion-review workflow that can turn a specific staged relationship into a person entity only when legal, privacy, provenance, and analyst-review requirements are satisfied.
