...
- Phase D - Public/product publication layer: about 18-24% is done.
- Overall long-term product: about 42-52% done, because the internal evidence spine is strong and the first presentable local product surface now exists, but the public product and more cross-domain Paraguay sources are still early.
...
- Add a real graph visualization over the existing graph-ready network endpoint so entity relationships become visually explorable, not only JSON/exportable.
- Add clearer queue filters and source-pack readiness controls inside the Command Center.
- Make case/source-pack artifacts easier to browse directly from the interface without leaving the local safety model.
...
- First presentable local/internal product surface: first meaningful slice achieved; likely 2-4 more focused interface runs before it feels like a strong internal MVP.
- First serious Paraguay internal MVP: likely 4-8 more focused implementation runs.
