...
- `sql/postgres/018_analyst_evidence_links.sql`
  - creates `centinela.analyst_evidence_links` and `centinela.analyst_case_evidence_overview` so source records can be linked to case targets and optional notes with field-level explanations and limitations
- `sql/postgres/019_case_evidence_exports.sql`
  - creates `centinela.analyst_case_public_reviews`, evidence export views, public review status on case overview, and public-safety timeline events
