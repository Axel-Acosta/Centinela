...
- the first analyst-workspace hardening slice adds token-protected saved cases/notes, source-record drilldowns, graph exports, and a case timeline workbench
...
- `sql/postgres/017_analyst_case_timeline.sql`
  - creates `centinela.analyst_case_timeline`, a unified case event view for case creation, linked targets, and case-scoped notes
...
- The first interactive API/console slice is local-only. It now supports token-protected notes/cases, case timelines, source-record drilldowns, and graph export, but not production authentication, role-based permissions, or public deployment posture yet.
...
The next investigation-layer milestone should link source records and field-level explanations directly into casework, then add case evidence bundles and eventually production-grade authentication before any public deployment.
