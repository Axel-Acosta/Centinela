...
- `docs/methodology/analyst-workspace.md` saved notes, cases, source-record drilldowns, and graph export rules
...
The internal console command serves a local-only analyst surface and JSON API for entity search, dossiers, graph-ready network neighborhoods, graph export, source-record drilldowns, review queues, external candidates, accepted matches, and token-protected analyst notes/cases.
...
- The first analyst-workspace hardening slice is live in schema and API: source-record drilldowns, graph exports, saved cases, case links, and saved analyst notes. Write endpoints require `CENTINELA_WRITE_TOKEN`.
