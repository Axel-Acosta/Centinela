  ...
  return withDatabase(async (client, schema) => {
    const counts = await client.query<Record<string, unknown>>(
      `select
         ...
         (select count(*) from ${schema}.entity_relationship_staging_reviews)::int as staged_relationship_reviews,
         (select count(*) from ${schema}.entity_relationship_staging_reviews where decision = 'promote_to_redacted_relationship')::int as promoted_staged_relationships,
  ...
  return withDatabase(async (client, schema) => {
    ...
    const stagedRelationships = await client.query<Record<string, unknown>>(
      `select
         ...
         latest_review_limitations,
         promoted_object_entity_id::text,
         promoted_relationship_id::text,
         source_url
    ...
    for (const row of stagedRelationships.rows) {
      ...
      addNode({
        ...
        metadata: {
          ...
          promotedObjectEntityId: row.promoted_object_entity_id,
          promotedRelationshipId: row.promoted_relationship_id,
        ...
        withConfidence(
          {
            ...
            metadata: {
              ...
              promotionStatus: row.promotion_status,
              latestReviewDecision: row.latest_review_decision,
              latestReviewRationale: row.latest_review_rationale,
              promotedRelationshipId: row.promoted_relationship_id,
